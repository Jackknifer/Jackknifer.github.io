...
每条动态的图片单独放在以该动态文件名（不含扩展名）命名的子目录里，例如动态 `2025-07-26.md` 的图片放在 `source/images/moments/2025-07-26/`。再在动态文件中加入：

```yaml
images:
  - src: /images/moments/2025-07-26/第一张.jpg
    alt: 第一张照片的说明
  - src: /images/moments/2025-07-26/第二张.jpg
    alt: 第二张照片的说明
```
...
网易云歌曲链接会在构建时自动读取歌曲名、歌手和封面，并显示为可直接播放的紧凑音乐卡；读取失败时会安全回退为仍可打开和播放的通用音乐卡，不会中断博客构建。若需要手动指定歌曲信息，也可以使用完整格式：
